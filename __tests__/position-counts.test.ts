...
test('_positionCounts - properly updates if put is called', () => {
    const chess = new Chess()
    expect(chess._positionCounts[DEFAULT_POSITION]).toBe(1)
    chess.move('e4')
    expect(chess._positionCounts[e4Fen]).toBe(1)
    ...
    chess.move('e5')
    expect(chess._positionCounts['rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPPPPPP/RNBQKBNR w KQkq - 0 3']).toBe(1)
})

test('_positionCounts - properly updates if remove is called', () => {
    const chess = new Chess()
    expect(chess._positionCounts[DEFAULT_POSITION]).toBe(1)
    chess.move('e4')
    expect(chess._positionCounts[e4Fen]).toBe(1)
